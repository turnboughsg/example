...
// Define the structure of a painting object
interface Painting {
    src: string;
    title: string;
    artist: string;
...
const Paintings: React.FC = () => {
    const [currentPainting, setCurrentPainting] = React.useState<Painting>(PAINTINGS[0]);
    const [image, setImage] = React.useState<string>(PAINTINGS[0].src);
    const [frame, setFrame] = React.useState<boolean>(true);
    ...
    const handleAddToCart = () => {
        ...
        alert(
            `Added to cart!\n${quantity} ${size} "${currentPainting.title}" by ${currentPainting.artist} with ${frame ? 'a' : 'no'} frame`
        );
    };
